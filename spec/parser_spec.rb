...
Message: "042" cannot be used as a name'
        syntax_err = raise_error(Lispetit::SyntaxError, expected_error_message)
        expect { parser.parse_token('042', 0) }.to syntax_err
      end
    end
...
Message: "042.3" cannot be used as a name'
        syntax_err = raise_error(Lispetit::SyntaxError, expected_error_message)
        expect { parser.parse_token('042.3', 0) }.to syntax_err
      end
    end
...
Message: "0invalid" cannot be used as a name'
        expect { parser.parse_token('0invalid', 0) }.to(
          raise_error(Lispetit::SyntaxError, expected_error_message)
        )
      end
    end
  end
...
'''
      ...
      children = ast.children.first.children
      expect(children[0]).to be_an_instance_of(ASTName).and have_attributes(value: 'name1')
      expect(children[1]).to be_an_instance_of(ASTList).and have_attributes(
      ...
      expect(children[2]).to be_an_instance_of(ASTList).and have_attributes(
        children: containing_exactly(
